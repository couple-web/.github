...
- `@Scheduled` 기반 스케줄러가 매일 새벽 실행되어  
  **3일 전** 일정 자동 감지
- 감지된 일정의 사용자의 지역 정보를 바탕으로  
  **OpenWeather API**를 호출해 날씨 확인
...
- 사용자 감성을 기반으로 한 추천이 필요한 경우  
  **Hugging Face AI API**와 연결해 유사 감성 장소 추출
- 추천 결과는 JSON 형태로 반환되고 프론트에서 카드 UI로 시각화

### 💬 4. AI 챗봇 연동

- `ChatBotController`를 통해 Hugging Face API 호출  
...
- 관리자 전용 통계 페이지 제공 (`/admin/stat`)
- 주요 기능:
  - 전체 사용자 수, 전체 장소 수, 전체 일정 수
  - 월별 방문자 수
  - 가장 인기 많은 장소/카테고리
- 통계 데이터는 Repository에서 직접 SQL 집계 처리  
  → DTO 변환 후 JSON으로 응답
- 프론트에서는 Chart.js / Recharts로 시각화

### 📦 6. 파일 업로드 및 이미지 처리

- 장소/앨범 등에서 이미지 업로드 시,  
  `MultipartFile`을 통해 서버에 저장
...
- 채팅/좋아요/팔로우/날씨 등 다양한 이벤트에 통합 대응
...
- ✅ **캘린더 기능**: `Calendar`, `ToDoList`, `AnniversaryList`, `FollowList`
